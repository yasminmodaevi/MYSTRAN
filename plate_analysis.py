...
class MeshGenerator:
    ...
    def generate(self):
        ...
        for i, (x, y, z) in enumerate(self.nodes):
            if np.isclose(x, 0): self.edge_nodes['left'].append(i)
            if np.isclose(x, self.a): self.edge_nodes['right'].append(i)
            if np.isclose(y, 0): self.edge_nodes['bottom'].append(i)
            if np.isclose(y, self.b): self.edge_nodes['top'].append(i)
        gmsh.finalize()
    ...
    def get_matrices(self, coords):
        ...
        for xi in gauss:
            for eta in gauss:
                ...
                # Bending (Standard thin plate)
                Bb = np.zeros((3, 8))
                for i in range(4):
                    Bb[0, 2*i+1] = dN[0,i]   # d(thy)/dx
                    Bb[1, 2*i]   = -dN[1,i]  # -d(thx)/dy
                    Bb[2, 2*i]   = -dN[0,i]  # -d(thx)/dx
                    Bb[2, 2*i+1] = dN[1,i]   # d(thy)/dy
...
class FEModel:
    ...
    def solve_static(self):
        free = np.delete(np.arange(self.num_dofs), self.fixed_dofs)
        K_free = self.K[free, :][:, free].tocsr()
        # print(f"Solving K_free {K_free.shape} with load max {np.max(np.abs(self.F[free]))}")
        u = np.zeros(self.num_dofs)
        if K_free.shape[0] > 0:
            u[free] = spsolve(K_free, self.F[free])
        return u

    def solve_modes(self, n=5):
        free = np.delete(np.arange(self.num_dofs), self.fixed_dofs)
        vals, vecs = eigsh(self.K[free, :][:, free], k=n, M=self.M[free, :][:, free], sigma=1.0, which='LM')
        results = []
        for i in range(n):
    ...
    def solve_buckling(self, n=5):
        u_s = self.solve_static()
        if np.allclose(u_s, 0):
             self.F[self.mesh.edge_nodes['right'][0]*6] = -1.0; u_s = self.solve_static()
        ...
        if k <= 0: return [], []
        try:
            eigvals, vecs = eigsh(A, k=k, M=B, sigma=1e-4, which='LM', tol=1e-4, v0=np.random.rand(len(free)))
        except:
            from scipy.linalg import eigh
            eigvals, vecs = eigh(A.toarray(), B.toarray())
            idx = np.argsort(np.abs(eigvals))[::-1]; eigvals = eigvals[idx[:k]]; vecs = vecs[:, idx[:k]]
        valid = np.abs(eigvals) > 1e-10; factors = np.ones(len(eigvals)) * np.nan; factors[valid] = 1.0 / eigvals[valid]
        modes = []
        for i in range(len(eigvals)):
            u = np.zeros(self.num_dofs); u[free] = vecs[:, i]; modes.append(u)
        return factors, modes

    def solve_nonlinear(self, steps=5):
        u = np.zeros(self.num_dofs); free = np.delete(np.arange(self.num_dofs), self.fixed_dofs)
...
class PlateAnalysisApp(QMainWindow):
    ...
    def init_gui(self):
        ...
        self.load_x_s = QLineEdit("0"); ll.addWidget(QLabel("X-Force Start [N]"), 1, 0); ll.addWidget(self.load_x_s, 1, 1)
        self.load_x_e = QLineEdit("0"); ll.addWidget(QLabel("X-Force End [N]"), 2, 0); ll.addWidget(self.load_x_e, 2, 1)
    ...
    def run_analysis(self):
        ...
        fxs, fxe = float(self.load_x_s.text()), float(self.load_x_e.text())
        if fxs != 0 or fxe != 0:
        ...
        if atype == "Linear Static": self.show_results(self.model.solve_static())
        elif atype == "Normal Modes": f, m = self.model.solve_modes(int(self.inputs['BM'].text())); print(f"Freqs: {f}"); self.show_results(m[0])
        elif atype == "Linear Buckling": fact, m = self.model.solve_buckling(int(self.inputs['BM'].text())); print(f"Buckling factors: {fact}"); self.show_results(m[0])
        elif atype == "Nonlinear Static": self.show_results(self.model.solve_nonlinear())

    def show_results(self, u):
        disp = u.reshape((-1, 6))[:, :3]; res = self.pv_mesh.copy()
        res.points = res.points.astype(float)
        res.points += disp * (1.0 / (np.max(np.abs(disp)) + 1e-9) * 50)
        res.point_data['Disp'] = np.linalg.norm(disp, axis=1)
        self.plotter.clear(); self.plotter.add_mesh(res, scalars='Disp', show_edges=True); self.plotter.reset_camera()
